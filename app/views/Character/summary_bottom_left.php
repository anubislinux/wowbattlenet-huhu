<div class="summary-bottom-left">
    <div class="summary-health-resource">
        <ul>
            <li class="health" id="summary-health" data-id="health"><span class="name">Health</span><span class="value"><?=$charInfo['real']['health'];?></span></li>
            <li class="resource-18" id="summary-power" data-id="power-18"><span class="name">Pain</span><span class="value">100</span></li>
        </ul>
    </div>
    <div class="summary-stats-profs-bgs">

    <div class="summary-stats" id="summary-stats">
        <div id="summary-stats-simple" class="summary-stats-simple" style=" display: block">
            <div class="summary-stats-simple-base">
    <div class="summary-stats-column">
        <h4>Характеристики</h4>
        <ul>
    <li data-id="strength" class="">
        <span class="name">Сила</span>
        <span class="value">8484</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="agility" class="">
        <span class="name">Ловкость</span>
        <span class="value color-q2">19452</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="intellect" class="">
        <span class="name">Интеллект</span>
        <span class="value">4997</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="stamina" class="">
        <span class="name">Выносливость</span>
        <span class="value color-q2">41089</span>
    <span class="clear"><!-- --></span>
    </li>
        </ul>
    </div>
    <div class="summary-stats-column">
        <h4>Атака</h4>
        <ul>
    <li data-id="attackdamage" class="">
        <span class="name">Урон</span>
        <span class="value">22886–25673</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="attackspeed" class="">
        <span class="name">Скорость атаки</span>
        <span class="value">2,41/2,41</span>
    <span class="clear"><!-- --></span>
    </li>
        </ul>
    </div>
    <div class="summary-stats-column">
        <h4>Заклинания</h4>
        <ul>
    <li data-id="manaregen" class=" no-tooltip">
        <span class="name">Восполнение маны</span>
        <span class="value color-q0">--</span>
    <span class="clear"><!-- --></span>
    </li>
        </ul>
    </div>
    </div>
    <div class="summary-stats-simple-other">
    <div class="summary-stats-column">
        <h4>Защита</h4>
        <ul>
    <li data-id="armor" class="">
        <span class="name">Броня</span>
        <span class="value">4294</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="dodge" class="">
        <span class="name">Уклонение</span>
        <span class="value">11,05%</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="parry" class="">
        <span class="name">Парирование</span>
        <span class="value">14,18%</span>
    <span class="clear"><!-- --></span>
    </li>
    <li data-id="block" class="">
        <span class="name">Блокирование</span>
        <span class="value">0,00%</span>
    <span class="clear"><!-- --></span>
    </li>
        </ul>
    </div>
    <div class="summary-stats-column">
        <h4>Усиления</h4>
        <ul>
    <li data-id="crit" class="">
        <span class="name">Критический удар</span>
        <span class="value">43,02%</span>
    <span class="clear"><!-- --></span>
    </li>







    <li data-id="haste" class="">
        <span class="name">Скорость</span>
        <span class="value">7,88%</span>
    <span class="clear"><!-- --></span>
    </li>







    <li data-id="mastery" class="">
        <span class="name">Искусность</span>
        <span class="value color-q2">14,06%</span>
    <span class="clear"><!-- --></span>
    </li>







    <li data-id="leech" class="">
        <span class="name">Cамоисцеление</span>
        <span class="value">0,00%</span>
    <span class="clear"><!-- --></span>
    </li>







    <li data-id="versatility" class="">
        <span class="name">Универсальность</span>
        <span class="value">3,10%</span>
    <span class="clear"><!-- --></span>
    </li>







    <li data-id="avoidance" class="">
        <span class="name">Избегание</span>
        <span class="value">0,00%</span>
    <span class="clear"><!-- --></span>
    </li>
        </ul>
    </div>
            </div>
            <div class="summary-stats-end"></div>
        </div>

    </div>

        <script type="text/javascript">
        //<![CDATA[
        $(document).ready(function() {
            new Summary.Stats({


            "health": 2514647,
            "power": 100,
            "powerTypeId": 18,
            "hasOffhandWeapon": true,
            "masteryName": "Искусность: оскверненная кровь",
            "masteryDescription": "\"Демонические шипы\" дополнительно уменьшают получаемый физический урон на 11,70%.\n\n\n\nТакже увеличивает вашу силу атаки на 15%.",
            "averageItemLevelEquipped": 837,
            "averageItemLevelBest": 837,
            "resistArcane_pet": -1,
            "resilience_damage": 0,
            "arcaneResist": 0,
            "armorPercent": 12.81446647644043,
            "expertiseOffPercent": 0,
            "critRating": 9456,
            "energy": 0,
            "dmgMainMax": 25673,
            "resistNature_pet": -1,
            "rangeAtkPowerBonus": 0,
            "shadowCrit": 42.01714324951172,
            "dmgMainDps": 10074.4970703125,
            "armor_petArmor": -1,
            "versatilityDamageDoneBonus": 3.1024999618530273,
            "spellDmg_petAp": -1,
            "rangedHaste": 7.8799967765808105,
            "mastery": 14.06357192993164,
            "spellPenetration": 0,
            "maelstrom": 0,
            "parryRatingPercent": 18.36116600036621,
            "dodgeRatingPercent": 0,
            "atkPowerLoss": 0,
            "parryRating": 9456,
            "spellHasteRatingPercent": 7.880000114440918,
            "lifestealRating": 0,
            "expertiseRating": 0,
            "bonusOffWeaponRating": 0,
            "agi_armor": 38904,
            "chi": 0,
            "sprTotal": 0,
            "bonusOffMainWeaponSkill": 1.1100000143051147,
            "rangeBonusWeaponRating": 0,
            "bonusMainWeaponSkill": 1.1100000143051147,
            "lunarPower": 0,
            "resistFrost_pet": -1,
            "dmgMainMin": 22886,
            "holyResist": 0,
            "bonusMainWeaponRating": 0,
            "parry": 14.180789947509766,
            "sturdinessRatingBonus": 0,
            "arcaneDamage": 4997,
            "agiTotal": 19452,
            "avoidanceRating": 0,
            "dmgRangeDps": -1,
            "lifesteal": 0,
            "focus": 0,
            "manaRegenPerFive": 0,
            "dmgRangeMax": -1,
            "soulShards": 0,
            "armorTotal": 4294,
            "block": 0,
            "sta_petSta": -1,
            "spellDamage": 4997,
            "multistrike": 0,
            "spellHitPercent": 0,
            "multistrikeRatingBonus": 0,
            "fireDamage": 4997,
            "sta_hp": 2465340,
            "expertiseRatingPercent": 0,
            "pvpResilienceRating": 0,
            "spr_regen": 0,
            "health": 2514647,
            "expertiseMainPercent": 0,
            "blockRating": 0,
            "masteryRating": 3763,
            "pvpPowerDamage": 0,
            "holyDamage": 4997,
            "powerTypePower": 1000,
            "expertiseMain": 3,
            "int_crit": 0,
            "runicPower": 0,
            "intTotal": 4997,
            "sturdinessRating": 0,
            "bonusArmor": 0,
            "versatilityHealingDoneBonus": 3.1024999618530273,
            "rangeAtkPowerLoss": 0,
            "spellHasteRating": 2561,
            "critRatingPercent": 27.01714324951172,
            "rangeAtkPowerBase": 0,
            "spellCritRating": 9456,
            "atkPowerTotal": 23099,
            "staTotal": 41089,
            "rangeBonusWeaponSkill": 27.01714324951172,
            "rangeHitRatingPercent": 0,
            "speedRating": 8.06357192993164,
            "hasteRating": 2561,
            "int_sp": 4997,
            "defensePercent": 0,
            "armorPenetration": 0,
            "fireResist": 0,
            "dmgRangeMin": -1,
            "atkPowerBonus": 0,
            "rangedHasteRating": 2561,
            "dmgOffSpeed": 2.4100000858306885,
            "uiHitModifier": 0,
            "uiSpellHitModifier": 0,
            "spellCritRatingPercent": 27.01714324951172,
            "rangeCritPercent": 43.01714324951172,
            "spellHaste": 7.8799967765808105,
            "rangeAtkPowerTotal": 0,
            "agi_crit": 0,
            "spellHitRating": 0,
            "insanity": 0,
            "str_ap": 0,
            "expertiseOff": 3,
            "holyPower": 0,
            "pvpPowerHealing": 0,
            "pvpResilienceBonus": 0,
            "arcaneCrit": 42.01714324951172,
            "strTotal": 8484,
            "defense": 0,
            "resistShadow_pet": -1,
            "pvpResilienceBase": 0,
            "frostResist": 0,
            "dodge": 11.051258087158203,
            "resistHoly_pet": -1,
            "spellCritPercent": 42.01714324951172,
            "rangeHitRating": 0,
            "avoidanceRatingBonus": 0,
            "mana": 0,
            "ap_dps": 1649.9285888671875,
            "dmgMainSpeed": 2.4100000858306885,
            "agiBase": 9027,
            "arcaneCharges": 0,
            "resilience_crit": 0,
            "frostCrit": 42.01714324951172,
            "rangeCritRatingPercent": 27.01714324951172,
            "holyCrit": 42.01714324951172,
            "shadowResist": 0,
            "armorBase": 4294,
            "hasteRatingPercent": 7.880000114440918,
            "multistrikeRating": 0,
            "dmgOffMax": 12837,
            "dmgOffDps": 5037.42529296875,
            "natureDamage": 4997,
            "sprBase": 0,
            "frostDamage": 4997,
            "pain": 1000,
            "block_damage": 30,
            "resistFire_pet": -1,
            "versatility": 1241,
            "critPercent": 43.01714324951172,
            "rangeHitPercent": 0,
            "natureResist": 0,
            "shadowDamage": 4997,
            "pvpResilience": 0,
            "blockRatingPercent": 0,
            "power": 100,
            "spellHitRatingPercent": 0,
            "pvpPowerRating": 0,
            "dmgRangeSpeed": -1,
            "strBase": 8484,
            "fury": 1000,
            "haste": 7.8799967765808105,
            "armorPenetrationPercent": 0,
            "fireCrit": 42.01714324951172,
            "spellDmg_petSpellDmg": -1,
            "rage": 0,
            "versatilityDamageTakenBonus": 1.5512499809265137,
            "hitPercent": 0,
            "dodgeRating": 0,
            "agi_ap": 19452,
            "healing": 4997,
            "rap_petSpellDmg": -1,
            "masteryRatingBonus": 8.06357192993164,
            "hitRatingPercent": 0,
            "pvpPower": 0,
            "expertiseRanged": 3,
            "speedRatingBonus": 0,
            "rangeCritRating": 9456,
            "rangedHasteRatingPercent": 7.880000114440918,
            "staBase": 6260,
            "intBase": 4997,
            "atkPowerBase": 23099,
            "natureCrit": 42.01714324951172,
            "dmgOffMin": 11443,
            "hitRating": 0,
            "masteryBase": 6,
            "rap_petAp": -1,
            "manaRegenCombat": 0,
            "lifestealRatingBonus": 0,
            "str_block": -1,
    "foo": true
});
        });
        //]]>
        </script>

                    </div>
                </div>